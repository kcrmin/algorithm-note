...
```java
int[] vals = a.clone(); // 원본 보존: 입력 배열은 그대로 두고 압축 기준만 정렬
Arrays.sort(vals);
int m = 0;
int[] uniq = new int[n];
for (int i = 0; i < n; i++) {
    if (i == 0 || vals[i] != vals[i - 1]) uniq[m++] = vals[i]; // 중복 제거로 좌표 수 축소
}
int[] comp = new int[n];
for (int i = 0; i < n; i++) {
    int idx = Arrays.binarySearch(uniq, 0, m, a[i]); // O(log M) 매핑
    comp[i] = idx;
}
```
...
for (int x : sorted) {
    if (uniq.isEmpty() || uniq.get(uniq.size() - 1) != x) uniq.add(x); // 정렬 상태에서 중복 제거
}
HashMap<Integer, Integer> map = new HashMap<>();
for (int i = 0; i < uniq.size(); i++) map.put(uniq.get(i), i); // 값 -> 압축 인덱스 O(1)
int[] comp = new int[n];
for (int i = 0; i < n; i++) comp[i] = map.get(a[i]); // 매 질의 O(1)로 빠른 변환
```
