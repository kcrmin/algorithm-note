...
# Priority Queue
> **한 줄 요약:** "매 단계 최선 후보 1개"를 빠르게 꺼내야 할 때 쓰는 힙 기반 자료구조 (`add/poll` O(log N), `peek` O(1))

## Core Concept
* **When to use:**
  - 가장 작은/큰 값을 반복적으로 꺼내며 처리할 때
  - "가장 유리한 후보"를 매 단계 선택해야 할 때 (예: Dijkstra, 이벤트 시뮬레이션)
* **Key Point:**
  - 최소 힙: 가장 작은 값이 우선 (기본)
...
### Pattern C: 객체 생성 줄이기 (packed long)
- 정점 수 upper bound가 확실할 때, `(dist, v)`를 `long` 하나로 packing해 GC 부담 완화
```java
final int SHIFT = 20;                 // v < 2^20 가정 (약 100만)
final long MASK = (1L << SHIFT) - 1;

PriorityQueue<Long> pq = new PriorityQueue<>();

// push
long packed = (dist << SHIFT) | v;
pq.add(packed);

// pop
long cur = pq.poll();
long curDist = cur >> SHIFT;
int curV = (int) (cur & MASK);
```

## Pitfalls & Tips
- PriorityQueue는 "전체가 정렬된 컨테이너"가 아니다 (top만 보장)
- 커스텀 객체는 반드시 Comparator 제공 필요
- Comparator에서 `a.cost - b.cost`는 overflow 위험이 있으니 `Integer.compare(a.cost, b.cost)` 또는 `comparingInt` 사용
- Dijkstra처럼 같은 정점이 여러 번 들어가는 문제는 `poll` 시점에 stale 검사로 버리는 게 정석 (삽입 시 제거하려고 하면 느림)
